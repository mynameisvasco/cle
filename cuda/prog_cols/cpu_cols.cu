...
double process_matrix(int id, double *matrix, int order)
{
    ...
    for (row_id = 0; row_id < order; row_id++)
    {
        int row = mat + row_id * order;
        for (int i = 0; i < order; i++)
        {
            int current_row = mat + i * order;
            ...
            if (row_id == i)
            {
                result *= matrix[current_row + i];
                continue;
            }

            for (int j = i + 1; j < order; j++)
            {
                matrix[row + j] -= matrix[current_row + j] * matrix[row + i] / matrix[current_row + i];
            }
        }
    }
    return result;
